  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/auth/login', {
        ...
        body: JSON.stringify(formData), // No role needed - API will detect automatically
      })
      ...
      // Login successful - use the redirect URL provided by the API
      alert(`Welcome ${data.user.name || data.user.email}! Redirecting to your dashboard...`)
      
      // Redirect based on user role (determined by API)
        ...
        <div className="max-w-md w-full bg-white rounded-2xl shadow-2xl p-8">
          ...
          <div className="mt-8 text-center">
            <p className="text-gray-600">
              ...
              <Link href="/member-signup" className="text-green-600 hover:text-green-500 font-medium">
                Sign up here
              </Link>
            </p>
          </div>
          ...
          <div className="mt-6 p-4 bg-gray-50 rounded-lg">
            <p className="text-xs text-gray-600 text-center font-medium mb-2">Test Credentials:</p>
            <div className="text-xs text-gray-500 space-y-1">
              <div className="text-center">
                <strong>Member:</strong> member@luxylyfe.com / member123
              </div>
              <div className="text-center">
                <strong>Admin:</strong> admin@luxylyfe.com / admin123
              </div>
            </div>
          </div>
        </div>
